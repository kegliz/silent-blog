...
import (
	...
	"github.com/kegliz/silent-blog/internal/app"
...
func appMain() error {
	...
	go func() {
		if err := srv.Listen(conf.GetInt("port"), conf.GetBool("localonly"), conf.GetBool("tls"), conf.GetString("domain")); err != nil && err != http.ErrServerClosed {
			log.Fatalf("Listen: %s\n", err)
		}
	}()
